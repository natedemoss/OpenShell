...
[Provider profiles](/sandboxes/providers-v2) for the profile format and custom
profile workflow.
</Info>
...
<Warning>
Legacy provider attachment is fixed at sandbox creation time. Provider profiles add
`openshell sandbox provider attach` and `openshell sandbox provider detach` for
running sandboxes. See [Provider profiles](/sandboxes/providers-v2#attach-and-detach-providers)
...
rewrite. Refer to [Provider profiles](/sandboxes/providers-v2#understand-static-credential-endpoint-binding)
