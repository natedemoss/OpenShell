...
This override is used by gateway inference routing. It disables the built-in
public OpenAI endpoint policy and credential binding for direct sandbox
traffic, so the alternate-upstream key cannot be substituted at
`api.openai.com`. Import an endpoint-bearing custom profile if a sandbox must
contact the alternate upstream directly.
...
The override is route-only and does not add the local endpoint to sandbox
network policy.
