...
function verifyAccessToken(jwt) {
  ...
  if (parsed.payload.demo_token_use !== "final") {
    throw new Error("expected final token");
  }
...
http
  .createServer((req, res) => {
    try {
      ...
      console.log(
        `${SERVICE_NAME} accepted request path=${req.url} sub=${claims.sub} aud="${aud}" scope="${claims.scope}" client_id=${claims.client_id}`,
      );
      return text(
        res,
        200,
