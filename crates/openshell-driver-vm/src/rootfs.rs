...
///
/// Including this in the image-cache key makes local, uncommitted guest-leaf
...
pub fn extract_host_supervisor(path: &Path) -> Result<(), String> {
    ...
    fs::write(path, supervisor).map_err(|error| format!("write {}: {error}", path.display()))?;
    #[cfg(unix)]
    {
...
fn ensure_guest_init_ip(rootfs: &Path) -> Result<(), String> {
    ...
    // Guest init runs before the process leaf can enter its trusted helper
...
mod tests {
    ...
    fn prepare_sandbox_rootfs_rewrites_guest_layout() {
        ...
        assert!(init_script.contains("vm-guest /etc/openshell/vm-guest.json"));
