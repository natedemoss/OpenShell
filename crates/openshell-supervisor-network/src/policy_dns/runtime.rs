...
use openshell_isolation::contract::{DnsMediationSource, DnsTransport};
        ...
        let task = tokio::spawn(async move {
            ...
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    .map_err(|error| {
                        openshell_isolation::contract::BackendError::Process(format!(
                            "policy DNS response failed: {error}"
                        ))
                    });
                    let _ = query.response.send(response);
                });
            }
        });
