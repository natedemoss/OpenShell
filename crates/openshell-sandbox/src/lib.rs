...
pub async fn run_sandbox(
    ...
    if let Some(descriptor) = topology_descriptor {
        ...
        .await?;
        info!(
            backend = %admitted_backend_name,
            "Host network supervisor connected to isolation boundary"
        );
        ca_file_paths
            .lock()
        ...
        let ready = bound
            ...
            .await
            .map_err(|error| miette::miette!(error.to_string()))?;
        info!(backend = %admitted_backend_name, "Isolation boundary enforcement confirmed");
        ...
        let running = ready
            ...
            .await
            .map_err(|error| miette::miette!(error.to_string()))?;
        info!(backend = %admitted_backend_name, "Isolation boundary agent started");
        let boundary_access = openshell_supervisor_process::run::start_boundary_access(
            sandbox_id.as_deref(),
            openshell_endpoint.as_deref(),
        ...
        .await?;
        info!(backend = %admitted_backend_name, "Host supervisor access plane started");
    ...
    // Existing container topologies retain their legacy lifecycle. The VM
    // backend above is the prototype RFC 0012 implementation.
    #[cfg(target_os = "linux")]
            ...
            && let Some(writer) = process_control_writer.clone()
        {
            ...
            tokio::spawn(async move {
                match rx.await {
                    Ok(pid) => {
                        if let Err(err) =
                            sidecar_control::send_entrypoint_started(&writer, pid, String::new())
                                .await
                        {
    ...
    tokio::spawn(async move {
        while let Some(update) = updates.recv().await {
            match update {
                ...
                sidecar_control::ControlUpdate::MainProcessExitAck { .. } => {}
            }
        }
    })
}
